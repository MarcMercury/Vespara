...
final profilePhotosProvider = StateNotifierProvider<ProfilePhotosNotifier, ProfilePhotosState>((ref) {
  return ProfilePhotosNotifier(Supabase.instance.client);
});

/// Provider for fetching another user's photos (for ranking)
final userPhotosProvider = FutureProvider.family<List<ProfilePhoto>, String>((ref, userId) async {
...
final existingRankingProvider = FutureProvider.family<PhotoRanking?, String>((ref, rankedUserId) async {
  final supabase = Supabase.instance.client;
...
class ProfilePhotosState {
  ...
  ProfilePhoto? photoAtPosition(int position) {
    return photos.where((p) => p.position == position).firstOrNull;
  }
  ...
  int get totalRankingsReceived {
    return photos.fold(0, (sum, p) => sum + (p.score?.totalRankings ?? 0));
  }
}
...
class ProfilePhotosNotifier extends StateNotifier<ProfilePhotosState> {
  ...
  Future<bool> uploadPhoto(Uint8List imageBytes, int position, {String extension = 'jpg'}) async {
    ...
    try {
      ...
      await _supabase.storage
          .from('profile-photos')
          .uploadBinary(storagePath, imageBytes, fileOptions: FileOptions(
  ...
  Future<bool> applyAIRecommendation() async {
    ...
    if (recommendation == null || !recommendation.hasRecommendation) return false;
  ...
  void _generateRecommendation() {
    final photosWithScores = state.photos.where((p) => 
      p.score != null && p.score!.hasEnoughData
    ).toList();
    ...
    photosWithScores.sort((a, b) => 
      a.score!.averageRank.compareTo(b.score!.averageRank)
    );
    ...
    final avgConfidence = photosWithScores.fold(0.0, (sum, p) => 
      sum + p.score!.confidenceScore
    ) / photosWithScores.length;
    
    // Total rankings
    final totalRankings = photosWithScores.fold(0, (sum, p) => 
      sum + p.score!.totalRankings
    );
  ...
  List<String> _getInsights({ProfilePhoto? bestPhoto}) {
    ...
    if (totalRankings == 0) {
      ...
      insights.add('${5 - totalRankings} more rankings needed for AI recommendations');
    }
