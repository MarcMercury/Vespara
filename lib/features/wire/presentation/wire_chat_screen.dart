...
import '../widgets/wire_voice_recorder.dart';
...
class _WireChatScreenState extends ConsumerState<WireChatScreen> {
  ...
  Future<void> _loadMoreMessages() async {
    ...
    final messages = ref.read(wireProvider).messagesByConversation[widget.conversation.id];
  ...
  Widget build(BuildContext context) {
    ...
    final participants = wireState.participantsByConversation[widget.conversation.id] ?? [];
    final typingUsers = wireState.typingByConversation[widget.conversation.id] ?? [];
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            child: Icon(
              widget.conversation.isGroup ? Icons.group : Icons.chat_bubble_outline,
              color: VesparaColors.secondary,
              size: 48,
            ),
          ),
  ...
  Widget _buildVoiceButton() {
    return GestureDetector(
      ...
      child: Container(
        ...
        height: 48,
        decoration: BoxDecoration(
          color: _isRecordingVoice ? VesparaColors.tagsRed : VesparaColors.glow,
          shape: BoxShape.circle,
        ),
        child: Icon(
          Icons.mic_rounded,
  ...
  void _deleteMessage(WireMessage message) async {
