...
-- ============================================
-- 1. CREATE ALL TABLES FIRST (without cross-referencing policies)
-- ============================================
...
BEGIN
    ...
    VALUES (v_group.id, auth.uid(), 'active', 'member')
    ON CONFLICT (group_id, user_id) 
    DO UPDATE SET status = 'active', joined_at = NOW(), left_at = NULL;
    
    INSERT INTO public.conversation_participants (conversation_id, user_id, role, can_send_messages, can_add_members)
    VALUES (v_group.conversation_id, auth.uid(), 'member', TRUE, FALSE)
...
BEGIN
    ...
    IF v_member.role = 'creator' THEN
        UPDATE public.vespara_groups SET is_active = FALSE, updated_at = NOW() WHERE id = p_group_id;
        UPDATE public.group_members SET status = 'left', left_at = NOW() WHERE group_id = p_group_id;
        UPDATE public.conversation_participants SET is_active = FALSE, left_at = NOW() WHERE conversation_id = v_group.conversation_id;
        UPDATE public.conversations SET is_archived = TRUE, archived_at = NOW() WHERE id = v_group.conversation_id;
    ELSE
        UPDATE public.group_members SET status = 'left', left_at = NOW() WHERE group_id = p_group_id AND user_id = auth.uid();
        UPDATE public.conversation_participants SET is_active = FALSE, left_at = NOW() WHERE conversation_id = v_group.conversation_id AND user_id = auth.uid();
        UPDATE public.conversations SET participant_count = participant_count - 1, updated_at = NOW() WHERE id = v_group.conversation_id;
    END IF;
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM public.group_members WHERE group_id = p_group_id AND user_id = p_invitee_id AND status = 'active') THEN
        RAISE EXCEPTION 'User is already a member of this group';
    END IF;
    
    IF EXISTS (SELECT 1 FROM public.group_invitations WHERE group_id = p_group_id AND invitee_id = p_invitee_id AND status = 'pending') THEN
    ...
    INSERT INTO public.group_invitations (group_id, inviter_id, invitee_id, message)
    VALUES (p_group_id, auth.uid(), p_invitee_id, p_message)
    ...
    VALUES (
        ...
        jsonb_build_object('invitation_id', v_invitation_id, 'group_id', p_group_id, 'group_name', v_group.name, 'inviter_id', auth.uid()),
...
-- Function to delete a group (disbands it completely)
CREATE OR REPLACE FUNCTION public.delete_vespara_group(p_group_id UUID)
RETURNS BOOLEAN
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
    ...
    IF NOT FOUND THEN
        RAISE EXCEPTION 'Group not found or you are not the creator';
    END IF;
    ...
    UPDATE public.group_invitations SET status = 'declined', responded_at = NOW() WHERE group_id = p_group_id AND status = 'pending';
    
    -- Archive the conversation
    IF v_group.conversation_id IS NOT NULL THEN
        UPDATE public.conversation_participants SET is_active = FALSE, left_at = NOW() WHERE conversation_id = v_group.conversation_id;
        UPDATE public.conversations SET is_archived = TRUE, archived_at = NOW() WHERE id = v_group.conversation_id;
    END IF;
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        WHERE pubname = 'supabase_realtime' AND tablename = 'vespara_groups'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_publication_tables 
        WHERE pubname = 'supabase_realtime' AND tablename = 'group_invitations'
    ) THEN
