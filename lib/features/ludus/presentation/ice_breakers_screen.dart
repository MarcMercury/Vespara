...
class _IceBreakersScreenState extends ConsumerState<IceBreakersScreen>
  ...
  Widget _buildDiscoveryPhase(IceBreakersState state) => Container(
                ...
                child: Row(
                  children: [
                    IconButton(
                      ...
                      icon:
                          const Icon(VesparaIcons.back, color: Colors.white70),
                    ),
                    const Spacer(),
                    // Demo mode badge
                    if (state.isDemoMode)
  ...
  Widget _buildLobbyPhase(IceBreakersState state) => Container(
                ...
                Expanded(
                  child: _buildModeButton(
                    ...
                    onTap: () {
                      ...
                      ref
                          .read(iceBreakersProvider.notifier)
                          .setGameMode(IceGameMode.couple);
                    },
                  ),
                ),
  ...
  Widget _buildPlayerRow(IcePlayer player, int index) => Container(
  ...
  Widget _buildAddPlayerRow() => Container(
        margin: const EdgeInsets.only(bottom: 8),
        ...
        child: Row(
          children: [
            ...
            IconButton(
              ...
              icon: const Icon(VesparaIcons.add, color: IceColors.primary),
            ),
          ],
        ),
      );
  ...
  Widget _buildPlayingPhase(IceBreakersState state) {
    ...
    if (state.phase == IceGamePhase.timer &&
        (_cardTimer == null || !_cardTimer!.isActive)) {
      _cardTimer?.cancel();
      ...
      child: Column(
        children: [
          ...
          if (state.currentPlayer != null &&
              state.phase != IceGamePhase.playing)
            Container(
              padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 12),
              margin: const EdgeInsets.symmetric(horizontal: 24),
              decoration: BoxDecoration(
                color: card.isWild
                    ? IceColors.wildCard.withOpacity(0.2)
                    : IceColors.primary.withOpacity(0.1),
                borderRadius: BorderRadius.circular(12),
              ),
              child: Row(
                ...
                children: [
                  Text(
                    ...
                    style: TextStyle(
                      ...
                      fontWeight: FontWeight.w600,
                      color:
                          card.isWild ? IceColors.wildCard : IceColors.primary,
                    ),
                  ),
                  if (card.targetType == TargetType.pair &&
          ...
          Expanded(
            child: GestureDetector(
              ...
              onHorizontalDragEnd: state.isCardRevealed
                  ? (details) {
  ...
  Widget _buildCardFront(IceBreakersState state, IceCard card) => Padding(
        padding: const EdgeInsets.all(32),
        child: Column(
          children: [
            // Card type badge
  ...
  Widget _buildResultsPhase(IceBreakersState state) => Container(
  ...
  Widget _buildStatRow(String label, String value, Color color) => Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
          Text(label,
  ...
  Widget _buildEscalationPhase(IceBreakersState state) => Container(
