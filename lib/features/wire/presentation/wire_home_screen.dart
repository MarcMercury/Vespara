...
class _WireHomeScreenState extends ConsumerState<WireHomeScreen>
  ...
  Widget _buildHeader(BuildContext context, WireState wireState) {
    return Container(
  ...
  Widget _buildGroupsTab(BuildContext context, WireState wireState) {
    final groups = wireState.groupConversations
        .where((c) => _matchesSearch(c))
        .toList();
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        return _buildConversationTile(context, groups[index]);
      },
    );
  }
  ...
  Widget _buildArchivedTab(BuildContext context, WireState wireState) {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        return _buildConversationTile(context, archived[index], isArchived: true);
      },
    );
  }
    ...
    return Dismissible(
      ...
      confirmDismiss: (direction) async {
        if (direction == DismissDirection.startToEnd) {
          // Archive/Unarchive
          if (isArchived) {
            await ref.read(wireProvider.notifier).unarchiveConversation(conversation.id);
          } else {
            await ref.read(wireProvider.notifier).archiveConversation(conversation.id);
          }
  ...
  Widget _buildAvatar(WireConversation conversation) {
    return Stack(
      children: [
        Container(
          width: 56,
          ...
          child: conversation.avatarUrl != null
              ? ClipOval(
                  child: Image.network(
                    ...
                    fit: BoxFit.cover,
                    errorBuilder: (_, __, ___) => _buildAvatarPlaceholder(conversation),
                  ),
                )
  ...
  Widget _buildAvatarPlaceholder(WireConversation conversation) {
    return Center(
      child: Icon(
  ...
  void _showConversationOptions(BuildContext context, WireConversation conversation) {
    VesparaHaptics.mediumTap();
    showModalBottomSheet(
      context: context,
      backgroundColor: VesparaColors.surface,
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              if (conversation.isGroup)
                ListTile(
                  leading: const Icon(Icons.exit_to_app, color: VesparaColors.tagsRed),
