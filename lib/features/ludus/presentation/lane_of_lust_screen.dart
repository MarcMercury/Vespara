...
class _LaneOfLustScreenState extends ConsumerState<LaneOfLustScreen>
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen(laneOfLustProvider.select((s) => s.lastPlacementResult), (prev, next) {
      if (next != null) {
        if (next.success) {
          ...
          Container(
            ...
            child: const Row(
              ...
              children: [
                ...
                Text(
                  'First to 10 Cards Wins!',
                  style: TextStyle(color: LaneColors.gold, fontWeight: FontWeight.w600),
                ),
              ],
            ),
          ),
              ...
              child: ElevatedButton(
                onPressed: () {
                  if (_nameController.text.trim().isNotEmpty) {
                    ...
                    HapticFeedback.heavyImpact();
                    ref.read(laneOfLustProvider.notifier).hostGame(_nameController.text.trim());
                  }
                },
                ...
                child: const Text('CREATE GAME', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700)),
              ),
            ),
          ],
        ),
        ...
        builder: (context, scrollController) => SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 24),
              _buildHowToStep('1', 'The Deal', 'Each player starts with 3 cards in their Lane, sorted by Desire Index'),
              _buildHowToStep('2', 'Draw a Card', 'On your turn, you draw a Mystery Card - you see the scenario but NOT the index!'),
              _buildHowToStep('3', 'Place It', 'Drag the card to where you think it belongs in your sorted Lane'),
              _buildHowToStep('4', 'Reveal', 'The card flips to reveal its true Desire Index (1-100)'),
              _buildHowToStep('5', 'Success?', 'If you placed it correctly, it stays! If not, the next player can steal it'),
              _buildHowToStep('🏆', 'Win', 'First player to collect 10 cards in their Lane wins!'),
              const SizedBox(height: 24),
              const Center(
                child: Text(
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          Container(
            width: 32, height: 32,
            decoration: BoxDecoration(color: LaneColors.crimson, borderRadius: BorderRadius.circular(8)),
            child: Center(child: Text(number, style: const TextStyle(fontWeight: FontWeight.w700, color: Colors.white))),
          ),
  ...
  Widget _buildLobby(LaneOfLustState state) {
    return Container(
        ...
        children: [
          ...
          AnimatedBuilder(
            ...
            builder: (context, child) {
              return Container(
                ...
                decoration: BoxDecoration(
                  gradient: LinearGradient(colors: [LaneColors.crimson.withOpacity(0.2), LaneColors.gold.withOpacity(0.2)]),
                  borderRadius: BorderRadius.circular(20),
                  border: Border.all(color: LaneColors.crimson.withOpacity(0.5 + _pulseController.value * 0.3), width: 2),
                ),
                child: Column(
                  children: [
                    const Text('ROOM CODE', style: TextStyle(fontSize: 12, letterSpacing: 2, color: Colors.white54)),
                    const SizedBox(height: 8),
                    GestureDetector(
                      onTap: () {
                      ...
                      child: Text(
                        state.roomCode ?? '----',
                        style: const TextStyle(fontSize: 40, fontWeight: FontWeight.w900, letterSpacing: 8, color: LaneColors.crimson),
                      ),
                    ),
                  ],
                ),
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('PLAYERS (${state.players.length}/8)', style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w600, letterSpacing: 1, color: Colors.white54)),
            ...
            GestureDetector(
              ...
              child: AnimatedOpacity(
                ...
                child: Container(
                  ...
                  padding: const EdgeInsets.symmetric(vertical: 18),
                  decoration: BoxDecoration(
                    gradient: const LinearGradient(colors: [LaneColors.crimson, Color(0xFFFF6B6B)]),
                    borderRadius: BorderRadius.circular(16),
                  ),
                  child: const Row(
                    mainAxisAlignment: MainAxisAlignment.center,
          ...
          if (state.players.length < 2)
            const Padding(
              ...
              child: Text('Need at least 2 players', style: TextStyle(color: Colors.white38, fontSize: 14)),
            ),
        ],
      ),
    );
  ...
  Widget _buildPlayerCard(LanePlayer player, int index) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: player.isHost ? LaneColors.gold.withOpacity(0.5) : player.avatarColor.withOpacity(0.3)),
      ),
      child: Row(
        children: [
          Container(
            ...
            child: Center(child: Text(player.displayName[0].toUpperCase(), style: TextStyle(fontSize: 18, color: player.avatarColor, fontWeight: FontWeight.w700))),
          ),
          const SizedBox(width: 12),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(player.displayName, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: Colors.white)),
                if (player.isHost) const Text('HOST', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w600, color: LaneColors.gold, letterSpacing: 1)),
              ],
            ),
          ),
          if (!player.isHost)
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: TextField(
          ...
          decoration: const InputDecoration(hintText: 'Player name', hintStyle: TextStyle(color: Colors.white38)),
          autofocus: true,
        ),
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              if (_nameController.text.trim().isNotEmpty) {
                ref.read(laneOfLustProvider.notifier).addLocalPlayer(_nameController.text);
                Navigator.pop(context);
              }
            },
            style: ElevatedButton.styleFrom(backgroundColor: LaneColors.crimson),
  ...
  Widget _buildDealing(LaneOfLustState state) {
    return Container(
      ...
      child: Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 24),
            const Text('Dealing Cards...', style: TextStyle(fontSize: 24, fontWeight: FontWeight.w700, color: Colors.white)),
            const SizedBox(height: 16),
            const SizedBox(width: 100, child: LinearProgressIndicator(color: LaneColors.crimson, backgroundColor: LaneColors.surface)),
          ],
        ),
      ),
    );
  ...
  Widget _buildOpponentsBar(LaneOfLustState state) {
    final opponents = state.players.where((p) => p.id != state.currentPlayerId).toList();
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...opponents.map((p) => Expanded(
            child: Container(
              margin: const EdgeInsets.symmetric(horizontal: 4),
              padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 6),
              decoration: BoxDecoration(
                color: (state.currentPlayer?.id == p.id || state.stealingPlayer?.id == p.id)
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          '${p.laneLength} cards',
                          style: TextStyle(
                            color: p.laneLength >= 8 ? LaneColors.gold : Colors.white38,
                            fontSize: 10,
                            fontWeight: p.laneLength >= 8 ? FontWeight.w700 : FontWeight.normal,
                          ),
                        ),
                      ],
                    ),
  ...
  Widget _buildTurnIndicator(LaneOfLustState state) {
    ...
    if (isMyTurn) {
      ...
      final activePlayer = isStealing ? state.stealingPlayer : state.currentPlayer;
      if (isStealing) {
          ...
          children: [
            ...
            if (showResult)
              Container(
                ...
                decoration: BoxDecoration(
                  color: result.success ? LaneColors.success : LaneColors.failure,
                  borderRadius: BorderRadius.circular(20),
                ),
                child: Row(
                  mainAxisSize: MainAxisSize.min,
            ...
            Draggable<LaneCard>(
              ...
              feedback: Material(
                ...
                child: _buildMysteryCard(card, state.isRevealed, isDragging: true),
              ),
            ...
            if (state.gameState == LaneGameState.stealing && state.isMyTurn && !state.isRevealed)
              Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          Padding(
            ...
            child: Row(
              children: [
                const Text('MY LANE', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, letterSpacing: 1, color: Colors.white54)),
                const SizedBox(width: 8),
                Text('${cards.length}/${state.winTarget}', style: const TextStyle(fontSize: 12, color: LaneColors.gold, fontWeight: FontWeight.w700)),
              ],
            ),
          ),
  ...
  Widget _buildMyStatsBar(LanePlayer me, LaneOfLustState state) {
    return Container(
      padding: const EdgeInsets.all(12),
      decoration: BoxDecoration(
        color: LaneColors.surface,
  ...
  Widget _buildGameOver(LaneOfLustState state) {
    ...
    final rankings = [...state.players]..sort((a, b) => b.laneLength.compareTo(a.laneLength));
            ...
            builder: (context, child) {
              return Container(
                ...
                decoration: BoxDecoration(
                  ...
                  boxShadow: [
                    BoxShadow(
                      color: LaneColors.gold.withOpacity(0.3 + _glowController.value * 0.3),
                      blurRadius: 40 + _glowController.value * 20,
                      spreadRadius: 10,
                    ),
                  ],
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return Container(
                  ...
                  padding: const EdgeInsets.all(12),
                  decoration: BoxDecoration(
                    color: isMe ? player.avatarColor.withOpacity(0.2) : LaneColors.surface,
                    borderRadius: BorderRadius.circular(12),
                  ...
                  child: Row(
                    children: [
                      ...
                      Container(
                        ...
                        child: Center(child: Text(player.displayName[0].toUpperCase(), style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w700))),
                      ),
                      const SizedBox(width: 12),
                      Expanded(child: Text(player.displayName, style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: isMe ? player.avatarColor : Colors.white))),
                      Text('${player.laneLength} cards', style: TextStyle(color: index == 0 ? LaneColors.gold : Colors.white54)),
                    ],
                  ),
                );
              },
            ...
            children: [
              Expanded(
                child: GestureDetector(
                  ...
                  child: Container(
                    padding: const EdgeInsets.symmetric(vertical: 16),
                    decoration: BoxDecoration(color: LaneColors.surface, borderRadius: BorderRadius.circular(16)),
                    child: const Center(child: Text('EXIT', style: TextStyle(color: Colors.white54, fontWeight: FontWeight.w600))),
                  ),
                ),
              ),
              ...
              Expanded(
                ...
                child: GestureDetector(
                  ...
                  child: Container(
                    ...
                    child: const Center(child: Text('PLAY AGAIN', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: 16))),
                  ),
                ),
              ),
            ],
